...
| 主机卸载（naive） | 参数/Adam 常驻 CPU 内存，每步整批传 GPU 并搬回梯度，用显存换规模（仅 default 策略） |
...
### 12.11 主机卸载（host-offload 分支）

| 参数 | 默认值 | 含义 |
|---|---|---|
| `--offload` | none | 主机卸载模式：`none`=全 GPU；`naive`=参数/Adam 在 CPU，每步整批传 GPU 并搬回梯度；`clm`=保留关键属性在 GPU、SH 分页（未实现） |
...
（`--sparse-grad` 会被自动关闭）。`--offload clm` 尚未实现。

压力测试结论（`--face-size 512 --sh-degree 2`，6 面同时渲染，
`--bench-gaussians N` 膨胀点云）：
...
- 8GB 显存下大场景增密后显存吃紧，可先用 packed+sparse-grad 缓解，
  或开启 `--offload naive`（本分支实验功能，见 14 节压力测试）。
